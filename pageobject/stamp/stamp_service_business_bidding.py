...
# @Time    : 2021/9/6 10:22
# @Author  : Mr_Li
# @FileName: stamp_service_business_bidding.py
...
logger.info('用印审批--商务招投标')


class BusinessBidding(BasePage):
    """用印审批--商务招投标"""
    ...
    # 选择文件类型：商务招投标
    stamp_service_business_bidding_choose = (By.XPATH, "//li[contains(text(),'10、商务招投标')]")
    # 输入交易对方
    ...
    # 是否关联方：是
    stamp_service_relevance_yes = (By.XPATH, "//div[@value-key='id']//span[text()='是']")
    # 是否关联方：否
    stamp_service_relevance_no = (By.XPATH, "//div[@value-key='id']//span[text()='否']")
    ...
    def test_business_bidding(self, username, password, value1, value2, value3, value4,
        ...
        sleep(1)
        logger.info('点击合同管理')
        self.click(BusinessBidding.contract_administration)
        logger.info('点击用印管理')
        self.click(BusinessBidding.using_management)
        logger.info('点击用印审批')
        self.click(BusinessBidding.stamp_service)
        sleep(1)
        # 选择用印单位
        logger.info('点击选择用印单位')
        self.click(BusinessBidding.stamp_service_user_center)
        logger.info('选择登录用户对应的用印单位')
        self.click(BusinessBidding.stamp_service_user_center1)
        # 选择紧急程度
        logger.info('选择用印紧急程度')
        self.click(BusinessBidding.stamp_service_emergency_degree)
        # 选择文件类型
        logger.info('输入“商务招投标”')
        self.send_ks(BusinessBidding.stamp_service_file_type, value1)
        logger.info('点击选择文件类型10、商务招投标')
        self.click(BusinessBidding.stamp_service_business_bidding_choose)
        logger.info('输入用印事由名称')
        self.send_ks(BusinessBidding.stamp_service_cause, value2)
        sleep(1)
        logger.info('输入交易对方')
        self.send_ks(BusinessBidding.stamp_service_adverse, value3)
        logger.info('点击选择输入的交易对方')
        self.click(BusinessBidding.stamp_service_choose_adverse)
        sleep(1)
        logger.info('输入项目名称')
        self.send_ks(BusinessBidding.stamp_service_project_name, value4)
        logger.info('点击对应项目名称')
        self.click(BusinessBidding.stamp_service_click_project_name)
        logger.info('点击涉及金额')
        self.click(BusinessBidding.stamp_service_money_involved)
        logger.info('输入项目金额')
        self.send_ks(BusinessBidding.stamp_service_input_money, value5)
        logger.info('选择是公司模板')
        self.click(BusinessBidding.stamp_service_corporate_yes)
        logger.info('选择是关联')
        self.click(BusinessBidding.stamp_service_relevance_yes)
        logger.info('选择印章类型')
        self.click(BusinessBidding.stamp_service_seal_type)
        logger.info('选择印章类型：公章')
        self.click(BusinessBidding.stamp_service_seal_type_choose)
        logger.info('点击印章公司')
        self.click(BusinessBidding.stamp_service_seal_company)
        logger.info('选择印章公司：1、矩阵纵横设计股份有限公司')
        self.click(BusinessBidding.stamp_service_seal_company_choose)
        logger.info('输入文件份数')
        self.send_ks(BusinessBidding.stamp_service_file_attachments, value6)
        logger.info('输入情况说明')
        self.send_ks(BusinessBidding.stamp_service_presentation, value7)
        logger.info('上传文件')
        self.send_ks(BusinessBidding.stamp_service_accessory, value8)
        sleep(3)
        logger.info('点击提交审批按钮')
        self.click(BusinessBidding.submit_stamp_service)
        self.jp_qd()

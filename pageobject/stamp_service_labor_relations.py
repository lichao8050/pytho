...
# @Time    : 2021/9/6 10:16
# @Author  : Mr_Li
# @FileName: stamp_service_labor_relations.py
...
logger.info('用印审批--劳动关系证明文件')


class LaborRelations(BasePage):
    """劳动关系证明文件--用印审批"""
    ...
    def test_labor_relations(self, username, password, value1, value2, value3, value4, value5):
        login = LoginPage(self.driver)  # 登录
        login.login_erp(username, password)  # 输入用户名、密码

        sleep(1)
        logger.info('点击合同管理')
        self.click(LaborRelations.contract_administration)
        logger.info('点击用印管理')
        self.click(LaborRelations.using_management)
        logger.info('点击用印审批')
        self.click(LaborRelations.stamp_service)
        sleep(1)
        # 选择用印单位
        logger.info('点击选择用印单位')
        self.click(LaborRelations.stamp_service_user_center)
        logger.info('选择登录用户对应的用印单位')
        self.click(LaborRelations.stamp_service_user_center1)
        # 选择紧急程度
        logger.info('选择用印紧急程度')
        self.click(LaborRelations.stamp_service_emergency_degree)
        # 选择文件类型
        logger.info('点击文件类型')
        self.click(LaborRelations.stamp_service_file_type)
        # 点击选择文件类型5、劳动关系证明文件
        sleep(1)
        logger.info('点击选择文件类型5、劳动关系证明文件')
        self.click(LaborRelations.stamp_service_labor_relations)
        logger.info('输入用印事由名称')
        self.send_ks(LaborRelations.stamp_service_cause, value1)
        sleep(1)
        logger.info('点击文件明细')
        self.click(LaborRelations.stamp_service_labor_relations_file)
        logger.info('选择在职证明')
        self.click(LaborRelations.stamp_service_labor_relations_file1)
        sleep(1)
        logger.info('输入收件单位全称')
        self.send_ks(LaborRelations.stamp_service_labor_relations_file_inbox, value2)

        logger.info('选择印章类型')
        self.click(LaborRelations.stamp_service_seal_type)
        logger.info('选择印章类型：公章')
        self.click(LaborRelations.stamp_service_seal_type_choose)
        logger.info('点击印章公司')
        self.click(LaborRelations.stamp_service_seal_company)
        logger.info('选择印章公司：1、矩阵纵横设计股份有限公司')
        self.click(LaborRelations.stamp_service_seal_company_choose)
        logger.info('输入文件份数')
        self.send_ks(LaborRelations.stamp_service_file_attachments, value3)
        logger.info('输入文件用途说明')
        self.send_ks(LaborRelations.stamp_service_presentation, value4)
        logger.info('上传文件')
        self.send_ks(LaborRelations.stamp_service_accessory, value5)
        sleep(3)
        logger.info('点击提交审批按钮')
        self.click(LaborRelations.submit_stamp_service)
        sleep(1)
